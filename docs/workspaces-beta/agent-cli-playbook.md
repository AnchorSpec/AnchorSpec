# AnchorSpec CLI Playbook For Agents
...
anchorspec initiative list --json
anchorspec initiative show <store>/<initiative> --json
anchorspec workspace doctor --json
```
...
Humans can run `anchorspec context-store setup` and answer prompts. Agents should
pass the setup inputs explicitly.

```bash
anchorspec context-store setup team-context --no-init-git --json
anchorspec context-store setup team-context --path /path/to/team-context --init-git --json
```
...
```bash
anchorspec initiative create billing-launch --store team-context --title "Billing Launch" --summary "Get billing live without losing the plot."
```
...
1. Resolve the workspace with `anchorspec workspace doctor --json`.
2. Resolve the initiative with `anchorspec initiative show <store>/<initiative> --json`.
...
```bash
anchorspec new change add-billing-api --initiative team-context/billing-launch
```
...
```bash
anchorspec workspace doctor --workspace billing-launch --json
anchorspec context-store doctor --json
```
