...
Org-level config (Settings → Secrets and variables → Actions, org scope, visible to `AnchorSpec/AnchorSpec`): variable `RELEASE_APP_ID` (numeric App ID, used by the workflow) and secret `RELEASE_APP_PRIVATE_KEY` (the App's private key). The workflow uses `app-id`, **not** `client-id` — `client-id` is `actions/create-github-app-token`'s documented recommendation, but empirically fails against this App with `'Issuer' claim ('iss') must be an Integer` (a real GitHub API rejection, not a lint warning — tried and reverted 2026-07). `RELEASE_CLIENT_ID` is also kept on the org for reference but unused. To manage the App itself: `AnchorSpec` org → Developer settings → GitHub Apps → AnchorSpec Release (owned by the org, not a personal account) — must be **installed** on `AnchorSpec/AnchorSpec` (Install App tab on the App's own settings page). Permissions needed: Contents (read/write), Pull requests (read/write). Revoke access by uninstalling the App from the repo, not by deleting the secret/variable.
